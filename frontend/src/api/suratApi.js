...
// GET surats by userId — filter di client karena MockAPI tidak support query params
export const getSuratsByUser = async (userId) => {
  const res = await getSurats();
  return { ...res, data: res.data.filter((s) => s.userId === userId) };
};

// GET surats by userId + jenisSurat — filter di client
...
// PER JENIS — konstanta jenis surat
export const JENIS = {
...
// Helper: GET by jenis + userId — filter di client
