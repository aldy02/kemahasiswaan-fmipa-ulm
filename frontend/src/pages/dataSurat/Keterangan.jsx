  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        <thead>
          <tr>
            {["No surat", "Nama", "NIM", "Prodi", "Keperluan", "Status", "Action"].map((h) => (
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.id} className="hover:bg-slate-50 transition-colors duration-100">
              ...
              <td className="px-4 py-3.5 text-sm font-semibold text-primary-1 border-b border-slate-50 max-w-50 truncate">{item.keperluan}</td>
  ...
  return (
    ...
    >
      ...
      {[
        ...
        <div key={label} className="flex justify-between text-[13px] mb-1.5 last:mb-0 gap-4">
          <span className="text-neutral-1 shrink-0">{label}</span>
          <span className="text-primary-1 font-semibold text-right">{value}</span>
        </div>
      ))}
...
export default function Keterangan() {
  ...
  const filtered = keteranganData.filter((d) =>
  ...
  return (
    <MainLayout>
      <div className="px-4 pt-5 pb-0 lg:px-8 lg:pt-7">
        <p className="hidden lg:block text-sm text-neutral-1 mb-1">Data Surat / Keterangan</p>
