...
export default function FormIzinTidakMengikutiKuliah() {
  ...
  useEffect(() => {
    ...
    getSuratById(id).then((res) => {
      ...
      setF({
        namaDosen: d.namaDosen || "", mataKuliah: d.mataKuliah || "", prodiDosen: d.prodiDosen || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        mahasiswas: mhsItems, namaDosen: f.namaDosen, mataKuliah: f.mataKuliah, prodiDosen: f.prodiDosen,
        tanggalMulai: toApiDate(f.tanggalMulai), tanggalAkhir: toApiDate(f.tanggalAkhir),
      ...
      if (isEdit) {
        ...
        await createSurat({
          ...payload, jenisSurat: "Izin Tidak Mengikuti Kuliah", userId: user?.id,
  ...
  const reset = () => {
    ...
    setF({ namaDosen: "", mataKuliah: "", prodiDosen: "", tanggalMulai: "", tanggalAkhir: "", sebab: "", tempat: "", keterangan: "" });
    setErrors(INIT_ERR);
  };
    ...
    <FormCard key="alasan" icon={<Info size={18} />} title="Alasan & Tempat Izin" subtitle="Sebab dan lokasi terkait izin tidak mengikuti kuliah">
      <FormGrid cols={2}>
        <FormField label="Sebab" required error={errors.sebab}>
          <FormTextarea placeholder="Masukkan alasan tidak mengikuti kuliah" value={f.sebab} onChange={set("sebab")} rows={3} error={errors.sebab} />
        </FormField>
